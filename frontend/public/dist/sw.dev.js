...
importScripts('./config.js'); //* Importando a lib idb.js no [SW] para podermos usar o IndexedDB com Promises também no ServiceWorker

importScripts('./src/js/idb.js'); //* Importando o ajudador que foi criado para inicializar o IndexedDB e também escrever e ler informações nos ObjectStores(tabelas) contidos no DB

importScripts('./src/js/indexedDB.js');
var CACHE_STATIC_NAME = 'static-v3';
var CACHE_DYNAMIC_NAME = 'dynamic-v3';
var CACHE_IMG = 'img-v3';
var STATIC_FILES = [BASE_URL + '/', BASE_URL + '/index.html', BASE_URL + '/my-posts.html', BASE_URL + '/offline.html', BASE_URL + '/config.js', BASE_URL + '/src/js/app.js', BASE_URL + '/src/js/checkIsLoggedUser.js', BASE_URL + '/src/js/feed.js', BASE_URL + '/src/js/sweetalert2.js', BASE_URL + '/src/js/promise.js', BASE_URL + '/src/js/fetch.js', BASE_URL + '/src/js/idb.js', BASE_URL + '/src/js/indexedDB.js', BASE_URL + '/src/js/helperFunctions.js', BASE_URL + '/src/css/normalize.css', BASE_URL + '/src/css/style.css', BASE_URL + '/src/css/fontello.css', BASE_URL + '/src/font/fontello.eot', BASE_URL + '/src/font/fontello.svg', BASE_URL + '/src/font/fontello.ttf', BASE_URL + '/src/font/fontello.woff', BASE_URL + '/src/font/fontello.woff2', 'https://fonts.googleapis.com/css2?family=Roboto:wght@100;300;400;500;700;900&display=swap', BASE_URL + '/src/images/logos/logo.png', BASE_URL + '/src/images/products/product-default.png'];
/**
...
function isInArray(string, array) {
  ...
  if (string.indexOf(BASE_URL) === 0) {
    // Requisições no próprio domínio, ou seja, quando não não requisições a um CDN ou outro domínio
    console.log('matched ', BASE_URL, string); // cachePath = string.substring(BASE_URL.length); // Tirando a parte do domínio na url e deixando apenas o que vem depois do domínio (e.g. after localhost:8080)
...
self.addEventListener('install', function (event) {
  ...
  event.waitUntil(caches.open(CACHE_STATIC_NAME).then(function (cache) {
    console.log('[Service Worker] Precaching App Shell'); //Fazendo requisição e armazenando todas as url marcadas no STATIC_FILES no cache
...
self.addEventListener('activate', function (event) {
  ...
  event.waitUntil( //!Verificando se algum dos caches pertence a alguma versão que não está dentre as atuais definida pelo nome da key do cache em CACHE_STATIC_NAME e CACHE_DYNAMIC_NAME
...
self.addEventListener('fetch', function (event) {
  console.log('[Service Worker] Fetching something', event.request.url); //! 1ª ESTRATÉGIA: [INDEXED-DB THEN NETWORK]
  ...
  var url = "".concat(API_BASE_URL, "/posts");
  var urlFirstPage = "".concat(API_BASE_URL, "/posts?limit=5&page=1"); //* Verificando se a URL requisitada é uma das que contém conteúdos que mudam constantemente
  ...
  if (event.request.url.indexOf(url) > -1) {
    ...
    if (event.request.url.indexOf(urlFirstPage) > -1) {
      ...
      event.respondWith(fetch(event.request).then(function (response) {
        ...
        var clonedResponse = response.clone(); //Limpando todas informações no respectivo ObjectStore(Tabela) 'posts' dentro IndexedDB antes de inserir as novas informações vindas da requisição na rede

        clearAllData('posts').then(function () {
          //Transformando o clone da resposta em um objeto JSON
          return clonedResponse.json();
        }).then(function (clonedResponseJSON) {
          var postsData = clonedResponseJSON.data; //Percorrendo cada JSON de CADA anúncio/post no retorno da requisição
      ...
      event.respondWith(fetch(event.request).then(function _callee(response) {
        var clonedResponse, postsData, key;
        return regeneratorRuntime.async(function _callee$(_context) {
          while (1) {
            switch (_context.prev = _context.next) {
              case 0:
                //Fazendo um clone da resposta da requisição
                clonedResponse = response.clone(); //Transformando o clone da resposta em JSON

                _context.next = 3;
                return regeneratorRuntime.awrap(clonedResponse.json());

              case 3:
                clonedResponseJSON = _context.sent;
                postsData = clonedResponseJSON.data; //Percorrendo cada JSON de CADA anúncio/post no retorno da requisição
        ...
        event.respondWith(caches.match(event.request).then(function (response) {
          ...
          if (response) {
            ...
            return fetch(event.request).then(function (res) {
              ...
              if (res.status == 404) throw ErrorEvent; //Se for requisição a uma imagem, salva no CACHE_IMG para que a limpeza de cache ocorra de acordo com o limite setado

              if (event.request.url.indexOf("".concat(API_BASE_URL, "/Images/")) > -1) {
                return caches.open(CACHE_IMG).then(function (cache) {
            ...
            ["catch"](function (error) {
              return caches.open(CACHE_STATIC_NAME).then(function (cache) {
                ...
                if (event.request.headers.get('accept').includes('text/html')) {
                  return cache.match("".concat(BASE_URL, "./offline.html"));
                } //Verificando se a requisição que falhou tentou puxar uma imagem de postagem, se for, retorna a imagem default de objeto
                else if (event.request.url.indexOf("".concat(API_BASE_URL, "/Images/")) > -1) {
                    return cache.match("".concat(BASE_URL, "/src/images/products/product-default.png"));
                  }
              });
            });
          }
...
self.addEventListener('sync', function (event) {
  ...
  console.log('[Service Worker] Background Syncing', event); //Verificando se a task registrada é uma 'sync-new-post'

  if (event.tag === 'sync-new-post') {
    console.log('[Service Worker] Syncing new Posts');
    event.waitUntil( //Lendo todas informações gravadas no ObjectStore(Tabela) sync-posts no IndexedDB
    readAllData('sync-posts').then(function (postsToSync) {
      ...
      try {
        for (var _iterator = postsToSync[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          ...
          postFormData.append('whatsapp_contact', post.whatsapp_contact); //Enviando a imagem e renomeando-a pois no servidor não podemos ter imagens com nmomes iguais

          postFormData.append('image', post.image, "".concat(post.id, ".png")); //Faz a requisição ao endpoint para sincronizar os posts do ObjectStore sync-posts com a API
              ...
              while (1) {
                switch (_context2.prev = _context2.next) {
                  ...
                  case 2:
                    ...
                    console.log('Inserido um novo post, seu id eh: ' + responseJSON.data.id_post); //? Enviando uma mensagem para a main thread do navegador no lado do client para atualizar os posts/anuncios usando a funcao fillPosts()
          ...
          })["catch"](function _callee3(errors) {
            var allClients;
...
self.addEventListener('notificationclick', function (event) {
  var notification = event.notification; //Pegando a notificação que contém a action disparada 

  var action = event.action; //Pegando propriedade action que representa o ID que identifica qual action(botão) foi clicada

  console.log(notification);

  if (action === 'confirm') {
    //Se o click foi na button(acion)==confirm
    console.log('Confirm was chosen');
    notification.close();
  } //A action see-post também entra no else
  else {
      //Se o click foi em qualquer lugar da Notification menos na button(action)==confirm
      console.log(action); //Abrindo a aplicação ou redirecionando para uma página específica caso a aplicação já esteja aberta

      event.waitUntil( //Pegando todos as janelas(os clients) abertas que estão sobre controle deste [SW]
      clients.matchAll().then(function (allClients) {
        //Buscando por uma janela(client) sob controle do SW que esteja aberta
        var clientOpened = allClients.find(function (c) {
          return c.visibilityState === 'visible';
        });

        if (clientOpened !== undefined) {
          //Verificando se encontrou algum client aberto
          clientOpened.navigate("".concat(BASE_URL).concat(notification.data.url)); //Redirecionando o client aberto para uma url da aplicação que foi setada pela API

          clientOpened.focus();
        } else {
          //Se não encontrou nenhum client aberto
          clients.openWindow("".concat(BASE_URL).concat(notification.data.url)); //Abrindo um client para uma url da aplicação que foi setada pela API
        }
...
self.addEventListener('push', function (event) {
  console.log('Web Push Notification received', event); //Setando uma configuração padrão para notificações caso não seja recebido o CONTEÚDO da WPN

  var data = {
    title: 'Novo anúncio!',
    content: 'Temos novidade para você no aplicativo!',
    openUrl: '/help.html'
  ...
  var options = {
    ...
    image: "".concat(BASE_URL, "/src/images/products/product-default-notification.png"),
    //Imagem a ser enviada no conteúdo
    ...
    data: {
      url: "".concat(BASE_URL + data.openUrl) //Passando a url que deve ser chamada quando o usuario clicar na notificacao
    ...
    actions: [{
      action: 'see-post',
      title: 'Ver',
      icon: "".concat(BASE_URL, "/src/images/icons/see-64x64.png")
    }, {
      action: 'confirm',
      title: 'Cancelar',
  ...
  event.waitUntil( //Acessando o registro desse SW no navegador para lançar as WPN
...
self.addEventListener('periodicsync', function (event) {
  ...
  if (event.tag === 'periodic-sync-posts') {
    ...
    var onPeriodicSync = function onPeriodicSync() {
      return regeneratorRuntime.async(function onPeriodicSync$(_context4) {
        while (1) {
