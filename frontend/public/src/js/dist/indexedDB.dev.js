...
function writeData(objectStoreName, dataToStore) {
  ...
  return dbPromise.then(function (db) {
    ...
    */
    var transaction = db.transaction(objectStoreName, 'readwrite'); //Abrindo o Object Store(Tabelas) após a transaction ser configurada acima

    var store = transaction.objectStore(objectStoreName); //Guardando cada POST/ANUNCIO no respectivo ObjectStore(Tableas)

    store.put(dataToStore); //Fechando a transaction [!OBS: Necessário apenas em transactions com permissão readwrite]
...
function readAllData(objectStoreName) {
  ...
  return dbPromise.then(function (db) {
     ...
     * 2º Parâmetro: Qual permissão ela tem para operar, readwrite ou readonly
    */
    var transaction = db.transaction(objectStoreName, 'readonly'); //Abrindo o Object Store(Tableas) após a transaction ser configurada acima

    var store = transaction.objectStore(objectStoreName); //Pegando todas informações no respectivo ObjectStore(Tableas)
...
function clearAllData(objectStoreName) {
  ...
  return dbPromise.then(function (db) {
     ...
     * 2º Parâmetro: Qual permissão ela tem para operar, readwrite ou readonly
    */
    var transaction = db.transaction(objectStoreName, 'readwrite'); //Abrindo o Object Store(Tableas) após a transaction ser configurada acima

    var store = transaction.objectStore(objectStoreName); //Limpando todas informações no respectivo ObjectStore(Tabelas)

    store.clear(); //Fechando a transaction [!OBS: Necessário apenas em transactions com permissão readwrite]
...
function deleteItemFromData(objectStoreName, id) {
  ...
  dbPromise.then(function (db) {
    ...
    */
    ...
    var store = transaction.objectStore(objectStoreName); //Deletando a informação do respectivo ID(PrimmaryKey) no respectivo ObjectStore(Tabelas)

    store["delete"](id); //Fechando a transaction [!OBS: Necessário apenas em transactions com permissão readwrite]
